...
cut -f 1,2 "${DATADIR}"/wordsim353/combined.tab | awk '{print tolower($0)}' | tr '\t' '\n' > "${DATADIR}"/ws_queries.txt

cat "${DATADIR}"/ws_queries.txt | ./fasttext print-word-vectors "${RESULTDIR}"/"${DATA}".bin "${RESULTDIR}"/ngrams.idf_sel > "${RESULTDIR}"/ws_vectors.txt

python3 eval.py -m "${RESULTDIR}"/ws_vectors.txt -d "${DATADIR}"/wordsim353/combined.tab
