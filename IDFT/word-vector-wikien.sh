...
./fasttext skipgram -input "${DATADIR}"/wiki.20M -output "${RESULTDIR}"/wiki20M -lr 0.025 -dim 100 \
  -ws 5 -epoch 1 -minCount 5 -neg 5 -loss ns -bucket 2000000 \
  -minn 3 -maxn 6 -thread 4 -t 1e-4 -lrUpdateRate 100 -idf ngrams.idf_20M

cut -f 1,2 "${DATADIR}"/rw/rw.txt | awk '{print tolower($0)}' | tr '\t' '\n' > "${DATADIR}"/queries.txt

cat "${DATADIR}"/queries.txt | ./fasttext print-word-vectors "${RESULTDIR}"/wiki20M.bin ngrams.idf_20M > "${RESULTDIR}"/vectors.txt
