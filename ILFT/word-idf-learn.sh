...
./fasttext skipgram -input "${DATADIR}"/"${DATA}" -output "${RESULTDIR}"/"${DATA}" -lr 0.025 -dim 300 \
  ...
  -minn 3 -maxn 6 -thread 10 -t 1e-4 -lrUpdateRate 100 -idf "${RESULTDIR}"/ngrams.idf_sel

cut -f 1,2 "${DATADIR}"/rw/rw.txt | awk '{print tolower($0)}' | tr '\t' '\n' > "${DATADIR}"/queries.txt

cat "${DATADIR}"/queries.txt | ./fasttext print-word-vectors "${RESULTDIR}"/"${DATA}".bin "${RESULTDIR}"/ngrams.idf_sel.learned > "${RESULTDIR}"/vectors.txt
