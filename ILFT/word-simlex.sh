...
cut -f 1,2 "${DATADIR}"/simlex/sl999.txt | awk '{print tolower($0)}' | tr '\t' '\n' > "${DATADIR}"/sl999_queries.txt

cat "${DATADIR}"/sl999_queries.txt | ./fasttext print-word-vectors "${RESULTDIR}"/"${DATA}".bin "${RESULTDIR}"/ngrams.idf_sel > "${RESULTDIR}"/simlex_vectors.txt

python3 eval.py -m "${RESULTDIR}"/simlex_vectors.txt -d "${DATADIR}"/simlex/sl999.txt
