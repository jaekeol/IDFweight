...
#./fasttext skipgram -input "${DATADIR}"/wiki.en.txt -output "${RESULTDIR}"/wikien -lr 0.025 -dim 100 \
...
#cat "${DATADIR}"/queries.txt | ./fasttext print-word-vectors "${RESULTDIR}"/wikien.bin ngrams.idf_sel > "${RESULTDIR}"/wi_vectors.txt

python3 eval.py -m "${RESULTDIR}"/wi_vectors.txt -d "${DATADIR}"/rw/rw.txt
