...
void FastText::skipgram(
  ...
  for (int32_t w = 0; w < line.size(); w++) {
    ...
    for (int32_t c = -boundary; c <= boundary; c++) {
      if (c != 0 && w + c >= 0 && w + c < line.size()) {
        model.update(ngrams, weights, line, w + c, lr, idf_);
      }
    }
  }
}
...
std::vector<std::pair<real, std::string>> FastText::getNN(
  ...
  // if k is minus, use all dictionary
  if( nwords > dict_->nwords() || nwords < 0) nwords = dict_->nwords();
  //for (int32_t i = 0; i < dict_->nwords(); i++) {
...
void FastText::writeIdf(const std::string& filename){
	int32_t wid;
	std::string ngram;
	std::string outfile = filename + ".learned";
	std::ofstream ofs(outfile);
	if(!ofs.is_open()){
		throw std::invalid_argument(
			outfile + " cannot be opened for writing idf!");
	}

	std::map<int32_t, float>::iterator iter;
	for(iter=idf_.begin(); iter != idf_.end(); iter++){
		wid = iter->first;
		if(inv_idx.find(wid) == inv_idx.end()) ngram = "NULL";
		else ngram = inv_idx[wid];
		ofs << ngram << "\t" << iter->second << "\t" << wid << std::endl;
	}
}
