  ...
  def load_idf(self, fname):
    ...
    for line in fp:
      ...
      if(len(psd) < 2): 
        continue   
      ng = psd[0].lower()
      h = hash(ng) % self.bucket
      nidx = self.nwords + h
      idv = float(psd[1])
      self.idf[nidx] = idv
      print("%s %d %f" % (ng, nidx, idv))
      idfcnt = idfcnt + 1
    print("%d idf read" % idfcnt) 
    ...
    for i, word in enumerate(self.id2word):
      ...
      for idx in ngram_idxs:
        ...
        if idx in self.idf:
          ...
          weight = 1.0
        subword_emb[i] += emb[idx] * weight
		weight_sum += weight
	  if(weight_sum == 0):
	    weight_sum = 1
      subword_emb[i] /= weight_sum*1.0
    self.cache=True
    ...
    if type(words) is str:
      ...
      for i, word in enumerate(words):
        if word not in self.word2id or not self.cache:
          ngram_idxs, ngram_s = self.getNgrams(word)
          ...
          for idx in ngram_idxs:
            if idx in self.idf:
              ...
              weight = 1.0
            vecs[i] += (emb[idx] * weight)
            weight_sum += weight
            #print("%s %d %f" % (word, idx, weight))
          if(weight_sum == 0):
            weight_sum = 1.0
          vecs[i] /= weight_sum*1.0
        else:
