...
void FastText::getVector_w(Vector& vec, const std::string& word) {  
  const std::vector<int32_t>& ngrams = dict_->getNgrams(word);
  vec.zero();
  float w;
  float wsum = 0;
...
void FastText::getVector_w2(Vector& vec, const std::string& word) {  
  const std::vector<int32_t>& ngrams = dict_->getNgrams(word);
  vec.zero();
  int32_t id = dict_->getId(word);
  if (id >= 0){
    vec.addRow(*output2_, id);
  }
}
...
void FastText::saveVectors() {
  ...
  Vector vec2(args_->dim);
  for (int32_t i = 0; i < dict_->nwords(); i++) {
    std::string word = dict_->getWord(i);
    //getVector(vec, word);
    getVector_w(vec, word);
    ofs << word << " " << vec << std::endl;
    //ofs << word << "\t" << vec << "\t" << vec2 << std::endl;
  }
  ofs.close();
}
