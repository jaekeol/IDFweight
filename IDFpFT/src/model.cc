...
void Model::computeHidden(const std::vector<int32_t>& input, const std::vector<float>& weights, Vector& hidden, bool dropout_dict, bool dropout_sub) const {
  assert(hidden.size() == hsz_);
  hidden.zero();
  int jjj = 0;
  float weight_sum = 0;
  ...
  for (int i=0; i<input.size(); i++){
    if(quant_) {
      ...
      if (!(args_->include_dictemb) && (jjj == 0)){
        ...
        if (!dropout_sub){
          //hidden.addRow(*wi_, *it);
          hidden.addRow(*wi_, input[i], weights[i]);
        }
      }
    }
  ...
  //if (!(args_->include_dictemb) and input.size() > 1) {
  if (!(args_->include_dictemb) and weight_sum > 1) {
    // make sure we're not dividing by zero
    hidden.mul(1.0 / (weight_sum - 1));
  } else {
    //hidden.mul(1.0 / input.size());
...
void Model::computeHidden2_mv(const std::vector<int32_t>& input, const std::vector<float>& weight, Vector& hidden) const {
  // Just using the vector for component 2
  assert(hidden.size() == hsz_);
  hidden.zero();
  int jjj = 0;
  //for (auto it = input.cbegin(); it != input.cend(); ++it) {
  for (int i=0; i<input.size(); i++){
    hidden.addRow(*wi2_, input[i], weight[i]);
    break;
  }
}
...
void Model::update(const std::vector<int32_t>& input, const std::vector<float>& weights, int32_t target, real lr) {
  assert(target >= 0);
  assert(target < osz_);
  if (input.size() == 0) return;
  ...
  //computeHidden(input, hidden_, false, false);
  computeHidden(input, weights, hidden_, false, false);
  computeHidden2_mv(input, hidden2_);
  //computeHidden2_mv(input, weights, hidden2_);
  if (args_->loss == loss_name::ns) {
    if (args_->multi){
      if (args_->var) {
        loss_ += negativeSamplingMultiVecVar(wordidx, target, lr);
  ...
  //for (auto it = input.cbegin(); it != input.cend(); ++it) {
  for (int i=0; i<input.size(); i++){
    wi_->addRow(grad_, input[i], weights[i]);
  }

  // MV mode - use only vector representation for cluster 2
  //for (auto it = input.cbegin(); it != input.cend(); ++it) {
  for (int i=0; i<input.size(); i++){
    wi2_->addRow(grad2_, input[i], weights[i]);
    break;
  }
  // update var
  if (args_->var){
